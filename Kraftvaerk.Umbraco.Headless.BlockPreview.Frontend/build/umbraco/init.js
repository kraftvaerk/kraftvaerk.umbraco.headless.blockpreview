...
export default function initUmbraco(args) {
    ...
    const solutionFilePath = join(rootDir, 'Kraftvaerk.Umbraco.Headless.BlockPreview.sln');
    ...
    try {
        ...
        execSync(`dotnet add "${join(projectDir, projectName + '.csproj')}" reference "${join(rootDir, 'Kraftvaerk.Umbraco.Headless.BlockPreview.Backend', 'Kraftvaerk.Umbraco.Headless.BlockPreview.Backend.csproj')}"`, { stdio: 'inherit' });
